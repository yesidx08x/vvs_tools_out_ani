...
class Project(ProjectCore):
    ...
    asset_work_path = 'R:/{id}_{project}/VFX/Assets/CGassets/{type}/{task}/{category}/Work/{project}_{task}_{abridge}_{alias}_{version}'
    asset_publish_path = 'R:/{id}_{project}/VFX/Assets/CGassets/{type}/{task}/{category}/Publish/{version}/{project}_{task}_{abridge}_{alias}'

    shot_work_path = 'R:/{id}_{project}/VFX/Sequences/{seq}/{shot}/{category_folder}/{category}/Work/{project}_{seq}_{shot}_{abridge}_{alias}_{version}'
    shot_publish_path = 'R:/{id}_{project}/VFX/Sequences/{seq}/{shot}/{category_folder}/{category}/Publish/{version}/{project}_{seq}_{shot}_{abridge}_{alias}'
    ...
    alias = ['','lod50', 'lod100', 'lod200', 'lod300', 'lod500','lod999']  # 群集50  远景100  中远200  近景300  特写500
    ...
    def get_publish_version(self, file_path, db_infos):
        ...
        if db_infos:
            ...
            new_file_path = Path(file_path.format(version=self.default_version, alias='{alias}',category_folder='{category_folder}')).parent.joinpath(
                base_name)
        return new_file_path, base_name, self.max_version.format(max_version)

    def get_asset_last_version(self, file_path, asset_type):
    ...
    def get_shot_last_version(self, file_path):
        ...
        category_name = self.file_infos['abridge']
        base_name = self.file_infos.get('base_name')
        db_shot_infos = self.api.get_shots_from_basename(project_id, category_name, base_name)
