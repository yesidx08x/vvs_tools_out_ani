...
class Project(ProjectCore):
    ...
    asset_work_path = 'R:/{id}_WYN_DJD/VFX/Assets/CGassets/{type}/{task}/{category}/Work/{project}_{task}_{abridge}_{alias}_{version}'
    asset_publish_path = 'R:/{id}_WYN_DJD/VFX/Assets/CGassets/{type}/{task}/{category}/Publish/{version}/{project}_{task}_{abridge}_{alias}'

    shot_work_path = 'R:/{id}_WYN_DJD/VFX/Sequences/{seq}/{shot}/{category_folder}/{category}/Work/{project}_{seq}_{shot}_{abridge}_{alias}_{version}'
    shot_publish_path = 'R:/{id}_WYN_DJD/VFX/Sequences/{seq}/{shot}/{category_folder}/{category}/Publish/{version}/{project}_{seq}_{shot}_{abridge}_{alias}'

    asset_review_path = 'R:/{id}_WYN_DJD/Assets/CGassets/{type}/{task}/Review/{category}/Publish/Internal/{project}_{task}_{abridge}_{alias}_{version}'
    shot_review_path = r'R:/{id}_WYN_DJD/VFX/Sequences/{seq}/{shot}/Review/{category}/Publish/Internal/{project}_{seq}_{shot}_{abridge}_{alias}_{version}'

    dailies_asset_path = 'R:/{id}_WYN_DJD/ProductionFolder/VFX_Dailies/Date/' + datetime.now().strftime(
        "%Y%m%d") + '/{category}/{project}_{task}_{abridge}_{alias}_{version}'
    dailies_shot_path = 'R:/{id}_WYN_DJD/ProductionFolder/VFX_Dailies/Date/' + datetime.now().strftime(
    ...
    asset_file_name_patterns = [
        r'^(?P<project>[^_]+)_(?P<task>[^_]+)_(?P<abridge>[^_]+)_(?P<version>v\d{4})',
        r'^(?P<project>[^_]+)_(?P<task>[^_]+)_(?P<abridge>[^_]+)_(?P<alias>[^_]+)_(?P<version>v\d{4})']

    shot_file_name_patterns = [
        r'^(?P<project>[^_]+)_(?P<seq>[^_]+)_(?P<shot>[^_]+)_(?P<abridge>[^_]+)_(?P<version>v\d{4})',
        r'^(?P<project>[^_]+)_(?P<seq>[^_]+)_(?P<shot>[^_]+)_(?P<abridge>[^_]+)_(?P<alias>[^_]+)_(?P<version>v\d{4})']
    ...
    model_template = format_path_join(os.path.dirname(__file__), 'template', 'model', 'model_template.ma')

    def __init__(self,dcc=None,pipeline=None,*args, **kwargs):
        super().__init__(*args, **kwargs)
        os.environ['model_templates_directory'] = self.model_template
        self.api = ServerAPI()
