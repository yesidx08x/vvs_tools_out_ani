...
from stage.common.utils import sanitize
...
class Tif(ExtractCore):
    ...
    def extract_tif(self, output_path):
        ...
        for layer in active_doc.Layers:
            if (len(active_doc.Layers)>1):
                for l in active_doc.Layers:
                    l.Visible = False
                layer.Visible = True
            ...
            output_path_tif=path.joinpath(base_name + sep+sanitize(layer_name)).with_suffix(suffix).as_posix()
            active_doc.SaveAs(output_path_tif, save_options, True)
            self._resolve_outputs.append(output_path_tif)
