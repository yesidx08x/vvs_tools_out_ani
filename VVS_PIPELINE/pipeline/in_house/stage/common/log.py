...
class Filelog:
    ...
    def __init__(self, logname = None, filename="stage", filedir=None, date=True, time=True, size_cap=500000):
        super(Filelog, self).__init__()
        self.file_name = filename if filename else "defaultLog"
        self.file_dir = filedir or os.path.join(os.path.dirname(os.path.dirname(__file__)),'logs')
