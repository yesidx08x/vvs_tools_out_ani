...
from stage.UI.widgets.common import IconButton
class FilterModel(QtCore.QSortFilterProxyModel):
    ...
    @QtCore.Slot(str)
    def set_filter_user(self, text: str):
        self._filter_user = str(text).lower()
        self.invalidate()
    ...
    def filterAcceptsRow(
        ...
        if self._ratio <= 0.0 or self._show_all or not self._filter_text and not self._filter_user:
        ...
        for column in  [0,3]:
            # Case-insensitive
            text = (
                self.sourceModel().index(source_row, column, source_parent).data() or ""
            ).lower()

            if not text:
                continue
            if not self._filter_user:
                if self._filter_text in text:
                    return True
            else:
                user = (
                        self.sourceModel().index(source_row, 4, source_parent).data() or ""
                ).lower()
                if self._filter_text in text and self._filter_user in user:
                    return True
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        filter_user = self._filter_user or ""
        ...
        if ratio < self._ratio and (filter_text or filter_user):
            # Precompute falloff
...
class FilterWidget(QtWidgets.QWidget):
    ...
    def __init__(self, filter_model):
        ...
        self.adv_button = IconButton(icon_name="arrow_right", size=22)
